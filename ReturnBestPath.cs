...
public class ReturnBestPath {
    ...
    public List<Edge> RunAlg(Floor cur_floor)
    {
      ...
      //  Debug.Log("goal = " + GoalCell.XLoc + "," + GoalCell.YLoc);
        ...
        //Debug.Log("start");
        Dfirst(Startcell, GoalCell,DefaultPath, newList, cur_floor,0);
       // Debug.Log("");
       // Debug.Log("SEARCH FINISHED");

        //bestPathLen = 0;
        for(int i = 1; i <visitedEdges.Count; i++)
        {
            //    bestPathLen = bestPathLen + getLenBetween(visitedEdges[i], visitedEdges[i - 1]);
            getLenBetween(visitedEdges[i -1], visitedEdges[i]);
        }
     
        //Debug.Log("edgefin");
        //Debug.Log("STATS:");
        //Debug.Log("edgeCount" + visitedEdges.Count);
        //Debug.Log("CellCount" + BestPath.Count);
        //Debug.Log("BestLen" + bestPathLen);
    ...
    private void Dfirst(Cell cur_cell, Cell GoalCell,List<Cell> curPath, List<Edge> pathEdges, Floor cur_floor,  int pthLen)
    {
        ...
        if (cur_cell != GoalCell && curPath.Count < cur_floor.cells.Count)
        {
                foreach (Edge e in cur_cell.edges)
                {
                    ...
                    {   
                            ...
                            if (visitedCheck(e.getRoomB(), NpthLen))
                            {
                                Dfirst(e.getRoomB(), GoalCell, NpathCells, NpathEdges, cur_floor,NpthLen);
                            }
                    }
                    else if (e.getSide() < 2 && !curPath.Contains(e.getRoomA()))
                    {
                            ...
                            int NpthLen = pthLen + getLenBetween(pathEdges[pathEdges.Count - 1],e);
                            if (visitedCheck(e.getRoomA(), NpthLen))
                            {
                                Dfirst(e.getRoomA(), GoalCell, NpathCells, NpathEdges, cur_floor, NpthLen);
                            }
                        }   
                }   
        ...
        {  
            if(cur_cell == GoalCell)
            {
                ...
                if (pthLen < bestPathLen || bestPathLen == 0)
                {
                    bestPathLen = pthLen;
